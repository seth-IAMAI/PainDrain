...
const bodyPartHotspots: { id: BodyPart; x: number; y: number; width: number; height: number; view: 'front' | 'back' }[] = [
    ...
    { id: 'right-shoulder', x: 25, y: 16, width: 10, height: 5, view: 'front' },
    // Viewer's right (Patient's Left)
    { id: 'left-shoulder', x: 65, y: 16, width: 10, height: 5, view: 'front' },
    // Viewer's left (Patient's Right)
    { id: 'right-arm', x: 18, y: 22, width: 15, height: 35, view: 'front' },
    // Viewer's right (Patient's Left)
    { id: 'left-arm', x: 67, y: 22, width: 15, height: 35, view: 'front' },
    // Viewer's left (Patient's Right)
    { id: 'right-hand', x: 15, y: 57, width: 10, height: 10, view: 'front' },
    // Viewer's right (Patient's Left)
    { id: 'left-hand', x: 75, y: 57, width: 10, height: 10, view: 'front' },
    // Viewer's left (Patient's Right)
    { id: 'right-leg', x: 35, y: 46, width: 15, height: 40, view: 'front' },
    // Viewer's right (Patient's Left)
    { id: 'left-leg', x: 50, y: 46, width: 15, height: 40, view: 'front' },
    // Viewer's left (Patient's Right)
    { id: 'right-foot', x: 35, y: 86, width: 15, height: 8, view: 'front' },
    // Viewer's right (Patient's Left)
    { id: 'left-foot', x: 50, y: 86, width: 15, height: 8, view: 'front' },
  ...
  return (
    <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 bg-secondary/30 p-4 rounded-lg">
      <div className="relative" data-ai-hint="body diagram female front">
        <Image src={frontImage} alt={`${gender} front view`} width={192} height={389} className="w-full h-auto" />
      ...
      <div className="relative" data-ai-hint="body diagram female back">
        <Image src={backImage} alt={`${gender} back view`} width={192} height={389} className="w-full h-auto" />
        <div className="absolute inset-0">
